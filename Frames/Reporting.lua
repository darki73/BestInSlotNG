...
function Reporting:CreateReport(reportType, text)
  ...
  window:AddChild(self:CreateUneditableTextbox("https://github.com/anhility/BestInSlotNG/issues/", L["Please navigate to the issue tracker."] .. " " .. (L["Please select the type: %s"]):format(reportType)))
...
local function renderBugSectionDump()
  ...
  local args = {
    ...
    GetAddOnMetadata("BestInSlotNG", "Version"), --5 the version the player is using
