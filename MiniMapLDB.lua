...
local BiSLDB = LibStub("LibDataBroker-1.1"):NewDataObject("BestInSlotNG", {
  type = "launcher",
  text = "Best In Slot NG",
  icon = "Interface\\Icons\\Achievement_ChallengeMode_Gold",
  OnTooltipShow = function(tooltip)
    tooltip:AddLine("Best In Slot NG")
...
function BestInSlot:RegisterMinimapIcon()
  minimapIcon:Register("BestInSlotNG", BiSLDB, BestInSlot.db.profile.minimap)
  isRegistered = true
end
...
function BestInSlot:MiniMapButtonVisible(bool)
  if bool then
    ...
    minimapIcon:Show("BestInSlotNG")
  else
    if not isRegistered then return end
    minimapIcon:Hide("BestInSlotNG")
  end
end
