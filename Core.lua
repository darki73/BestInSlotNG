...
local E = select(2, ...)
local BestInSlot = LibStub("AceAddon-3.0"):NewAddon("BestInSlotNG", "AceComm-3.0", "AceHook-3.0", "AceSerializer-3.0", "AceTimer-3.0")
...
--@end-non-debug@]===]
BestInSlot.AlphaVersion = not (GetAddOnMetadata("BestInSlotNG", "Version"):find("Release") and true or false)
--@do-not-package@
...
function BestInSlot:OnInitialize()
    self.db = LibStub("AceDB-3.0"):New("BestInSlotNGDB", defaults)
...
BestInSlot:RegisterSlashCmd("help", (L["%s - this dialog"]):format("/bis help"), function()
    ...
    DEFAULT_CHAT_FRAME:AddMessage(BestInSlot.colorHighlight..("-"):rep(5)..BestInSlot.colorNormal.."BestInSlotNG "..L["commands"]..BestInSlot.colorHighlight..("-"):rep(5).."|r")
    BestInSlot:Print(("%s: %s (%s)"):format(GAME_VERSION_LABEL, GetAddOnMetadata("BestInSlotNG", "Version"), BestInSlot.version))
    for i=1,#orderedList do
